...
public function add($quote_id = null)
{
    ...
    if ($this->form_validation->run() == true) {
        ...
        //previously checking if need be to send email confirmation but that was just so dumb, mail shold automatically be sent.
        /*$not_ware = $this->input->post('not_ware') ? 1 : 0;
        $not_cust = $this->input->post('not_cust') ? 1 : 0;*/
    ...
    if ($this->form_validation->run() == true && $this->sales_model->addSale($data, $products, $payment)) {
        ...
        $msg = 'Dear ' . $data['customer'] . ', Thank you for shopping with Technology Distribution LTD.\nYour commitment to us has not gone unnoticed and we value your patronage.\nIf you feel that something is missing or not properly calculated please feel free to send us a mail at sales@tdafrica.com or go to our website and file a support ticket. Thank you for your patronage.\nYour order reference number is: ' . $data['reference'];
        $this->sma->mailNotify('support@tdafrica.com', 'Sales Team', $customer_details->email, 'TDAFRICA says Thank You', $msg,$cc=$this->session->userdata('email'), $bcc=null);
