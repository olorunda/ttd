...
//generate pdf and force to download
    ...
    public function getExpenses()
    {
        ...
        $email_link = "<a href='#' onclick='notifySomeone($1)'><i class='fa fa-envelope-o'></i> ".lang('email_someone')."</a>";
    ...
    public function email_notification($id = null)
    {
        $this->sma->checkPermissions('index');
        ...
        if ( ! $expense_id ) {
            die('No expense report selected.');
        }
        ...
        $this->data['rows'] = $this->purchases_model->getExpenseByID($expense_id);
        ...
        $this->data['expense'] = $expense;
        $this->data['page_title'] = $this->lang->line("expense_note");
        $mailview = $this->load->view($this->theme . 'purchases/expense_note', $this->data, TRUE);

        if ($this->sma->send_email($to, 'Expense Report From: ' . $this->session->userdata('name'), $mailview)) {
