...
class Sma
{
    ...
    public function formatSAC($num)
    {
        ...
        public function mailNotify($from, $name, $to, $subject, $msg, $cc=null, $bcc=null)
        {
            $config = Array(
                'protocol'  => 'smtp',
                'smtp_host' => 'tls://smtp.office365.com',
                'smtp_port' => '587',
                'smtp_user' => 'support@snapnet.com.ng',
                'smtp_pass' => 'Foco5619',
                'mailtype'  => 'html',
                'starttls'  => true,
                'newline'   => "\r\n"
        ...
        {
            try {
                if(fopen("http://br-cdbr-azure-south-b.cloudapp.net:3306/", "r"))
                {
                    return "connected";
                }
            } catch(Exception $ie) {
        ...
        public function timeStatus()
        {
            ...
            $closing_date_time = new DateTime($current_date . ' 06:30:00');
            $closing_date_time = date_format($closing_date_time, 'Y-m-d H:i:s');
            //$time_difference = $closing_date_time->diff(new DateTime($current_date_time));

            $your_time = new DateTime($current_date_time);
            $your_time = date_format($your_time, 'Y-m-d H:i:s');

            $status = strcmp($closing_date_time, $your_time);
