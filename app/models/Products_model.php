...
class Products_model extends CI_Model
{
        ...
        if ($this->db->insert('products', $data)) {
            ...
            if ($data['type'] != 'standard') {
                foreach ($warehouses as $warehouse) {
                    $this->db->insert('warehouses_products', array('product_id' => $product_id, 'warehouse_id' => $warehouse->id, 'quantity' => 0, 'created_at' => date('Y-m-d H:i:s')));
                }
            }
            ...
            if ($warehouse_qty && !empty($warehouse_qty)) {
                foreach ($warehouse_qty as $wh_qty) {
                    if (isset($wh_qty['quantity']) && ! empty($wh_qty['quantity'])) {
                        $this->db->insert('warehouses_products', array('product_id' => $product_id, 'warehouse_id' => $wh_qty['warehouse_id'], 'quantity' => $wh_qty['quantity'], 'rack' => $wh_qty['rack'], 'avg_cost' => $data['cost'], 'created_at' => date('Y-m-d H:i:s')));
            ...
            if ($product_attributes) {
                foreach ($product_attributes as $pr_attr) {
                    ...
                    if ($pr_attr['quantity'] != 0) {
                        $this->db->insert('warehouses_products_variants', array('option_id' => $option_id, 'product_id' => $product_id, 'warehouse_id' => $variant_warehouse_id, 'quantity' => $pr_attr['quantity'], 'created_at' => date('Y-m-d H:i:s')));
                    ...
                    foreach ($warehouses as $warehouse) {
                        if (!$this->getWarehouseProductVariant($warehouse->id, $product_id, $option_id)) {
                            $this->db->insert('warehouses_products_variants', array('option_id' => $option_id, 'product_id' => $product_id, 'warehouse_id' => $warehouse->id, 'quantity' => 0, 'created_at' => date('Y-m-d H:i:s')));
                        }
                    }
    ...
    public function updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants)
    {
        if ($this->db->update('products', $data, array('id' => $id))) {
            ...
            if ($warehouse_qty && !empty($warehouse_qty)) {
                foreach ($warehouse_qty as $wh_qty) {
                    $this->db->update('warehouses_products', array('rack' => $wh_qty['rack']), array('product_id' => $id, 'warehouse_id' => $wh_qty['warehouse_id'], 'created_at' => date('Y-m-d H:i:s')));
                }
            }
            ...
            if ($product_attributes) {
                foreach ($product_attributes as $pr_attr) {
                    ...
                    if ($pr_attr['quantity'] != 0) {
                        $this->db->insert('warehouses_products_variants', array('option_id' => $option_id, 'product_id' => $id, 'warehouse_id' => $variant_warehouse_id, 'quantity' => $pr_attr['quantity'], 'created_at' => date('Y-m-d H:i:s')));
    ...
    public function updateProductOptionQuantity($option_id, $warehouse_id, $quantity, $product_id)
    {
        if ($option = $this->getProductWarehouseOptionQty($option_id, $warehouse_id)) {
            if ($this->db->update('warehouses_products_variants', array('quantity' => $quantity), array('option_id' => $option_id, 'warehouse_id' => $warehouse_id, 'created_at' => date('Y-m-d H:i:s')))) {
                $this->site->syncVariantQty($option_id, $warehouse_id);
                return TRUE;
            }
        } else {
            if ($this->db->insert('warehouses_products_variants', array('option_id' => $option_id, 'product_id' => $product_id, 'warehouse_id' => $warehouse_id, 'quantity' => $quantity, 'created_at' => date('Y-m-d H:i:s')))) {
                $this->site->syncVariantQty($option_id, $warehouse_id);
                return TRUE;
            }
        }
...
public function getProductQuantity($product_id, $warehouse)
{
    ...
    public function insertQuantity($product_id, $warehouse_id, $quantity, $rack = NULL)
    {
        ...
        if ($this->db->insert('warehouses_products', array('product_id' => $product_id, 'warehouse_id' => $warehouse_id, 'quantity' => $quantity, 'rack' => $rack, 'avg_cost' => $product->cost, 'created_at' => date('Y-m-d H:i:s')))) {
    ...
    public function updateQuantity($product_id, $warehouse_id, $quantity, $rack = NULL)
    {
        ...
        if ($this->db->update('warehouses_products', $data, array('product_id' => $product_id, 'warehouse_id' => $warehouse_id, 'created_at' => date('Y-m-d H:i:s')))) {
            $this->site->syncProductQty($product_id, $warehouse_id);
            return true;
        }
        return false;
    ...
    {
        if ($this->db->update('warehouses_products', array('rack' => $data['rack']), array('product_id' => $data['product_id'], 'warehouse_id' => $data['warehouse_id'], 'created_at' => date('Y-m-d H:i:s')))) {
            return TRUE;
        }
        return FALSE;
    }
    ...
    # this is where my additional codes begin.
    # you made update to this segment

    public function getWarehouseTableContent($table, $warehouse_id)
    {
        $q = $this->db->get_where($table, array('warehouse_id' => $warehouse_id));
        if($q->num_rows() > 0)
        {
            foreach(($q->result()) as $row)
            {
    ...
    {
        $q = $this->db->get_where('warehouses_products', array('warehouse_id' => $warehouse_id));
        if($q->num_rows() > 0)
        {
            foreach (($q->result()) as $row) {
                $data[] = $row;
    ...
    public function getWarehouseProductByDate($table, $column, $warehouse_id, $start_date, $end_date)
    {
        $this->db->where($column . ' >= ', $start_date);
        $this->db->where($column . ' <= ', $end_date);
        $this->db->where('warehouse_id', $warehouse_id);
        $q = $this->db->get($table);

        /*$q = $this->db->where('created_at BETWEEN "' . date('Y-m-d', strtotime($start_date)) . '" and "' . date('Y-m-d H:i:s', strtotime($end_date)) . '"')->where('warehouse_id', $warehouse_id);*/
    ...
    # sales synchronization functions

    public function getWarehouseSales($warehouse_id)
    {
    ...
    # purchases synchronization functions

    public function getWarehousePurchases($warehouse_id)
    {
        $q = $this->db->get_where('purchases', array('warehouse_id' => $warehouse_id));
        if($q->num_rows() > 0)
        {
            foreach (($q->result()) as $row) {
                $data[] = $row;
    ...
    public function getUpdatesCount($table)
    {
        ...
        # $table_row_count = $this->count_all('write_updates')->where('operation', $table);
        # $table_row_count = $this->db->count_all('write_updates');

        $this->db->where('operation', $table);
        $this->db->from('write_updates');
        $table_row_count = $this->db->count_all_results();
